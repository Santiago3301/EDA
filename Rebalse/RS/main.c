    ...
    for(int i=0;i<MAX;i++){
        if(isEmpty_lista_vinculada(t.tabla[i])){
            ...
            while(1){
                ...
                for (j = 0; j < (27 - strlen(eaux.correo)); j++)
                    printf(" ");
                printf("%c\n", vertical_line);
                // Nota
                ...
                for (j = 0; j < (24 - strlen(eaux.condicion)); j++)
                    printf(" ");
                printf("%c\n", vertical_line);
                ...
                if (isOos_lista_vinculada(t.tabla[i]))
                {
                    ...
                    printf("[    ]    %c%c> ", rightwards_T, horizontal_line);
            }
        }
        // printf("[    ]\n");
    }
...
int main()
{
    ...
    while (opc)
    {
        ...
        case 1:
            ...
            alta_tabla_hash(&tabla_hash, aux, &exito, &cant);
            if (exito == 1)
                printf("Estudiante cargado\n");
            else if (exito == 0)
                printf("No se pudo cargar el estudiante\n");
            else if (exito == -1)
                printf("No se pudo asignar memoria para el estudiante\n");
            pause();
            break;
        case 2:
            if (cant == 0)
        ...
        case 4:
            ...
            else
            {
                ...
                if (c == 1)
                    mostrarEstructura_long(tabla_hash);
                else
                    mostrarEstructura_short(tabla_hash);
            }
            pause();
            pause();
            break;
